...
func (s *Server) Handler(ctx *fasthttp.RequestCtx) {
	...
	switch mode {
	case configure.ModeNode, configure.ModeSignal:
	default:
		ctx.SetStatusCode(404)
		return
	}

	tkn, err := types.VerifyClientJoinToken(s.config, utils.B2S(ctx.Request.Header.Peek("authentication")))
	if err != nil {
	...
	case configure.ModeNode:
		handler = s.nodeHandler(tkn, ctx.Request.Header.Peek("node"))
...
func (s *Server) nodeHandler(tkn types.JoinTokenPayload, raw []byte) websocket.FastHTTPHandler {
	...
	return func(c *websocket.Conn) {
		...
		registerNode := func() {
			...
			s.node.SetNode(pl.Name, node.Node{JoinPayloadNode: pl})
	...
	return func(c *websocket.Conn) {
		...
		for {
			...
			case types.MessageTypeNodeRegister:
				...
				s.node.SetNode(pl.Node.Name, node.Node{JoinPayloadNode: pl.Node})
			...
			case types.MessageTypeSignalRegister:
				...
				if changed {
					...
					s.BroadcastNodes(msg)
					s.signal.Register(pl.Signal.SignalServer)
				}
