...
func (e *SignalEntry) Start(s *SignalStore) {
	s.wg.Add(1)
	defer s.wg.Done()
...
func (e *SignalEntry) read(msgs chan Message) {
	var (
		data []byte
		err  error
		msg  types.Message
...
func (e *SignalEntry) Write(msg types.Message) error {
...
func (e *SignalEntry) new(ctx context.Context, config *configure.Config, dialer websocket.Dialer) *websocket.Conn {
	var (
		conn *websocket.Conn
		err  error
	)
	...
	tkn, err := types.GenerateClientJoinToken(config, configure.ModeSignal, config.Name)
	if err != nil {
