...
func GenerateClientJoinToken(config *configure.Config, mode configure.Mode, name string) (string, error) {
	...
	data, _ := json.Marshal(JoinTokenPayload{
		CreatedAt: time.Now(),
		Mode:      mode,
		Name:      name,
	})
...
func VerifyClientJoinToken(config *configure.Config, tkn string) (JoinTokenPayload, error) {
	key, err := hex.DecodeString(config.TokenKey)
	if err != nil {
		return JoinTokenPayload{}, err
	}
	c, err := aes.NewCipher(key)
	if err != nil {
		return JoinTokenPayload{}, err
	...
	if len(ciphertext) < nonceSize {
		return JoinTokenPayload{}, ErrBadJoinToken
	}
